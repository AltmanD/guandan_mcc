...
def make_atari(env_id, frame_stack=False):
    env = gym.make(env_id)
...
class Monitor(gym.Wrapper):
    ...
    def update(self, ob, rew, done, info):
        ...
        if done:
            ...
            epinfo = {"reward": round(eprew, 6), "length": eplen}
            info['episode'] = epinfo
...
class WarpFrame(gym.ObservationWrapper):
    def __init__(self, env, width=84, height=84, grayscale=True):
        ...
        num_colors = 1 if self.grayscale else 3
        self.observation_space = spaces.Box(low=0, high=255,
                                            shape=(self.height, self.width, num_colors), dtype=np.uint8)
